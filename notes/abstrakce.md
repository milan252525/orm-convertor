...
|                   | Dapper                                     | PetaPoco                                    | RepoDB                                                         | linq2db                                                                                | NHibernate                                                                             | EF Core                                                                                |
| ----------------- | ------------------------------------------ | ------------------------------------------- | -------------------------------------------------------------- | -------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| zdrojová tabulka  | FROM v dotazu                              | FROM v dotazu nebo<br>\[TableName\]         | \[Table\]                                                      | builder.Entity<>().HasSchemaName("")<br>.HasTableName("")                              | `<class table="" schema="" ...>`                                                       | \[Table\]                                                                              |
| projekce          | SELECT ... FROM<br>+ musí odpovídat entitě | Sql.Builder automaticky dle entity          | Field.From("", "")                                             | mapování entity<br>+ LINQ .Select()<br>transformace z properties entity na jiný objekt | mapování entity<br>+ LINQ .Select()<br>transformace z properties entity na jiný objekt | mapování entity<br>+ LINQ .Select()<br>transformace z properties entity na jiný objekt |
| selekce           | WHERE ...                                  | WHERE ...<br>(Sql.Builder.Where)            | lambda funkce jako první parametr<br>(s => s.SupplierID == 10) | LINQ .Where()<br>přes properties entity                                                | LINQ .Where()<br>přes properties entity                                                | LINQ .Where()<br>přes properties entity                                                |
| spojování         | JOIN ...<br>ON ...                         | JOIN<br>(Sql.Builder<br>.LeftJoin().On(())) | nelze<br>(částečně přes QueryMultiple)                         | Relace přes property entity + mapování<br>použití LoadWith na danou property<br>       | Relace přes property entity + mapování<br>použití Fetch na danou property              | Relace přes property entity + mapování<br>použití Include na danou property            |
| seskupování       | GROUP BY<br>HAVING                         | GROUP BY<br>HAVING<br>(Sql.Builder.GroupBy) | nelze<br>(v paměti)                                            | LINQ .GroupBy<br>(having podle předcházejícího Where)                                  | LINQ .GroupBy<br>(having podle předcházejícího Where)                                  | LINQ .GroupBy<br>(having podle předcházejícího Where)                                  |
| množinové operace | UNION, INTERSECT<br>v sql                  | UNION, INTERSECT<br>v sql                   | UNION, INTERSECT<br>v sql                                      | LINQ query.Union(query)                                                                | LINQ query.Union(query)                                                                | LINQ query.Union(query)                                                                |
| řazení            | ORDER BY v sql                             | ORDER BY v sql                              | OrderField.Ascending()...                                      | LINQ .OrderBy                                                                          | LINQ .OrderBy                                                                          | LINQ .OrderBy                                                                          |

Poznámka: spojování alternativně LINQ metoda .Join()
Skip, Take, Limit, Distinct - vynecháno, buď v SQL nebo funkce v LINQ
