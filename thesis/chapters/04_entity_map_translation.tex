    ...
    \ForEach{\textup{$attr$ in $class$.GetAttributes()}}{
        \If{\textup{$attr$.Name == 'Table'}}{
            ...
            \ForEach{\textup{$arg$ in $attr$.GetArguments()}}{
                ...
                \If{\textup{$argName$ == null || $argName$ == 'Name'}}{
                    $table$ = $arg$.Value\;
                }
...
\begin{algorithm}[!p]
    ...
    \BlankLine
    $isNullable$ = $\neg hasRequiredAttr$ \&\& $type$.IsNullable\;
...
%The translation process consists of two phases: a decoupled parsing phase followed by a framework-aware building phase. This approach ensures that the final output fully preserves the semantics of the original entity and its mapping. The result conforms to NHibernate’s requirements while remaining adherent to the structure and intent of the original \acrshort{efcore} entity.
