...
We not only want to measure performance, but also assure the results are correct.
...
Unit tests should be easy to set up and run. Configuring a database can turn into a complicated task. We will create a Docker container running MSSQL. The dataset will be loaded on startup. Execution of unit tests will be just a matter of starting the Docker container and running the \texttt{dotnet test} command (or alternatively using Visual Studio). 
...
If no such query language is available, we will opt to use raw SQL. 
...
Logging will be turned off for performance tests. It can be advantageous to see generated SQL queries. So where possible, we will enable SQL logging into test output window where possible.
