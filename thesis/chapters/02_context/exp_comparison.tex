...
The primary goals are to validate the correctness of our previous analysis, measure performance, and assess how effectively each framework integrates into the .NET through its supported query language.
...
The database contains over thirty tables split into four schemas: \texttt{Application}, \texttt{Purchasing}, \texttt{Sales}, \texttt{Warehouse}. For each query, we will select a suitable table and columns that can best showcase the desired feature.

% database, docker, test project, .net
The dataset is made for Microsoft SQL Server (MSSQL). \footnote{Microsoft SQL Server is the third most popular database management system as of March 2025 according to DB-Engines Ranking\cite{DBEngines2025Ranking}. With a score of 788.14, it is behind Oracle (1253.08) and MySQL (988.13). The score takes into account the number of mentions on websites, frequency of technical discussions, number of job offers mentioning the system, relevance in social networks, and other parameters. \cite{DBEnginesRankingMetrics} } Microsoft SQL Server is commonly used in .NET applications with EF Core ORM as the three products are all developed by Microsoft.  
...
Our goal is to test the query capabilities as broadly as possible. We want to create queries that use different conditions, result modifications, aggregations, relationships between tables, etc. The queries are grouped into categories A-H by the type of features they test. \autoref{tab:queries-sum} provides an overview of each query purpose. 
We will go over each query and provide an example of what the query should look like in raw SQL. The example can be used to directly query the test database to preview results. The example might be simplified for this text. Unit tests might use a different query language where available.
...
\begin{tabular}{ll}
\toprule
\textbf{ID} & \textbf{Query purpose} \\ 
\midrule
A1 & Projection – entity identical to table \\ 
A2 & Limited projection – subset of columns \\ 
A3 & Split projection – one row to multiple entities \\ 
A4 & Stored-procedure projection \\ 
B1 & Selection over indexed column \\ 
B2 & Selection over non-indexed column \\ 
B3 & Range filter \\ 
B4 & \texttt{IN} list filter \\ 
B5 & Text search \\ 
B6 & Paging \\ 
C1 & Aggregation - count \\ 
C2 & Aggregation - max \\ 
C3 & Aggregation - sum with expression \\ 
D1 & One-to-many relationship\\ 
D2 & Many-to-many relationship \\ 
D3 & Optional one-to-many relationship \\ 
E1 & Sorting with limit \\ 
E2 & Distinct projection \\ 
F1 & JSON object query \\ 
F2 & JSON array query \\ 
G1 & Set union\\ 
G2 & Set intersection \\ 
H1 & Metadata lookup – column data type \\ 
\bottomrule
\end{tabular}
\end{table}

\subsubsection{A - Entity projection}
This group will test how well ORM can handle projecting table columns onto a user-defined entity or entities. Queries \textbf{A1}, \textbf{A2}, and \textbf{A3} will fetch one row based on its ID. The measured time and memory allocation will then show the overhead of the ORM framework when mapping data to the resulting entities. Query \textbf{A4} returns a large number of results, so it is the first query that shows us the performance with high-volume data.
...
\subsubsection*{A2 Subset of columns} \label{query:a2}
The resulting entity will have fewer properties than table columns. Only the data we really need should be transferred. 
Table \texttt{Purchasing.Suppliers} will be queried, and only columns related to the supplier's contact info will be retrieved.
...
\subsubsection*{A3 One row to multiple entities} \label{query:a3}
...
\subsubsection*{A4 Stored procedure result projection} \label{query:a4}
...
The query will retrieve one order line based on the ID of its order. The ID is a foreign key with an index built over it.
...
Unlike the first query, this one will fetch order lines filtered by a column without an index. The column that we will filter over will be the unit price of the order line.
...
\subsubsection*{B3 Range filter} \label{query:b3}
...
\subsubsection*{B4 IN list filter} \label{query:b4}
...
This query will test a text search, looking for any order lines with descriptions containing the word ``C++``.  The column does not have a full-text search or any other index.
...
This query will fetch all order lines ordered by ID, but will skip the first 1000 and take only the next 50.
...
Count is very useful when we need to know the number of records that correspond to a specific condition. We will combine it with \texttt{GROUP BY} clause to retrieve distinct tax rates and the number of their occurrences.
...
In this test, we will use the relation between an order and its order lines. We will fetch one specific order along with all its lines.
...
\subsubsection*{D3 Optional one-to-many relationship} \label{query:d3}
...
\subsubsection*{E1 Sorting with limit} \label{query:e1}
...
\subsubsection*{E2 Distinct projection} \label{query:e2}
...
\subsubsection{H - Metadata lookup}
...
The set is not exhaustive but representative enough to give us a fair idea of how different ORMs perform. 
...
To run performance benchmarks, set \texttt{BenchmarkMain} as a startup project. Right-click it in the Solution Explorer window and choose `Set as Startup project`. Make sure configuration is set to Release, not Debug. Then start the project without debugging (CTRL + F5 keys). A console window will appear. It contains instructions on how to target specific benchmarks. To run all, type in an asterisk (*) and press enter. This will trigger a full run, which takes approximately one hour to finish. You can switch to the test configuration which does only a few numbers of iterations in \texttt{BenchmarkMain/Program.cs} file. What the configuration does is further explained in the following sections. 
